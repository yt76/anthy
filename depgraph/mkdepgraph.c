...
get_node_id_by_name(const char *name)
{
  ...
  gNodes[nrNodes].follow_mask = 0;
  gNodes[nrNodes].branch = NULL;
...
find_branch(struct dep_node *node, xstr **strs, int nr_strs)
{
  ...
  for (i = 0; i < node->nr_branch; i++) {
    db = &node->branch[i];
    if (nr_strs != db->nr_cond_strs) {
      continue ;
    }
    for (j = 0; j < nr_strs; j++) {
      if (anthy_xstrcmp(db->cond_strs[j], strs[j])) {
	goto fail;
      }
    }
    /**/
  ...
  node->nr_branch++;
  db->cond_strs = malloc(sizeof(xstr*)*nr_strs);
  for (i = 0; i < nr_strs; i++) {
    db->cond_strs[i] = strs[i];
  }
  db->nr_cond_strs = nr_strs;
  db->nr_transitions = 0;
  db->transitions = 0;
  return db;
}
...
parse_dep(char **tokens, int nr)
{
  ...
  for ( ; row < nr; row++){
    ...
    db->transitions = realloc(db->transitions,
			      sizeof(struct dep_transition)*
			      (db->nr_transitions+1));
    tr = &db->transitions[db->nr_transitions];
...
static void
do_calc_follow_mask(struct dep_node *node)
{
  int i, mask;
  if (node->follow_mask) {
    /* 計算済み */
  ...
  for (i = 0; i < node->nr_branch; i++) {
    int j;
    int has_epsilon = 0;
    struct dep_branch *br = &node->branch[i];
    for (j = 0; j < br->nr_cond_strs; j++) {
      xstr *cond_xs = br->cond_strs[j];
      if (cond_xs->len == 0) {
	has_epsilon = 1;
      } else {
	/* 遷移条件の一文字目を取り出して、マスクに追加する */
	mask |= (1 << (cond_xs->str[0] & 31));
      }
    }
    /**/
    if (has_epsilon || br->nr_cond_strs == 0) {
      /* 遷移条件が空文字列の物があるので、遷移先のマスクを追加する */
      int k;
      for (k = 0; k < br->nr_transitions; k++) {
	/* 遷移先のマスク */
	int next_node_id = br->transitions[k].next_node;
	struct dep_node *next_node = &gNodes[next_node_id];
	do_calc_follow_mask(next_node);
	mask |= next_node->follow_mask;
	/**/
	if (next_node_id == 0) {
	  /* 終端への遷移 */
	  mask |= 1;
  ...
  /**/
  node->follow_mask = mask;
}

static void
calc_follow_mask(void)
{
  int i;
  for (i = 0; i < nrNodes; i++) {
    do_calc_follow_mask(&gNodes[i]);
  }
}
...
write_branch(FILE* fp, struct dep_branch* branch)
{
  ...
  /* 遷移条件 */
  write_nl(fp, branch->nr_cond_strs);
  for (i = 0; i < branch->nr_cond_strs; ++i) {
    write_xstr(fp, branch->cond_strs[i]);
  }

  /* 遷移先 */
  write_nl(fp, branch->nr_transitions);
  for (i = 0; i < branch->nr_transitions; ++i) {
    write_transition(fp, &branch->transitions[i]);
  }
}
...
/* 付属語グラフをファイルに書き出す */
static void
write_depgraph_file(const char* file_name)
{
  int i;
