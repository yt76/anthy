...
static void
sort_all_segments(struct segment_list *sl, int nth)
{
  int i;
...
static void
fixup_parenthesis_order(struct segment_list *sl, int nth)
{
  int i, j;
  for (i = 0; i < sl->nr_segments; i++) {
    struct seg_ent *seg = anthy_get_nth_segment(sl, i);
    xchar xc;
    if (seg->cands[0]->str.len != 1) {
      continue ;
    }
    xc = anthy_lookup_corresponding_parenthesis(seg->cands[0]->str.str[0]);
    if (!xc) {
      continue ;
    }
    for (j = nth; j < sl->nr_segments; j++) {
      struct seg_ent *target_seg = anthy_get_nth_segment(sl, j);
      int k;
      if (target_seg->str.len != 1) {
	continue ;
      }
      for (k = 0; k < target_seg->nr_cands; k++) {
	if (target_seg->cands[k]->str.len != 1) {
	  continue ;
	}
	if (target_seg->cands[k]->str.str[0] == xc) {
	  if (target_seg->cands[k]->score <
	      seg->cands[0]->score) {
	    /* 元の候補のスコアを与える */
	    target_seg->cands[k]->score = seg->cands[0]->score;
	  }
	  target_seg->cands[k]->score += 100;
	}
      }
    }
  }
...
anthy_sort_candidate(struct segment_list *sl, int nth)
{
  ...
  /* またソートする */
  sort_all_segments(sl, nth);

  /* カッコ閉じと開きの対応を付ける */
  fixup_parenthesis_order(sl, nth);
  sort_all_segments(sl, nth);
