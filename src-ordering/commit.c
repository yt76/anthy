...
#define MAX_OCHAIRE_ENTRY_COUNT 500
#define MAX_OCHAIRE_LEN 32
#define MAX_PREDICTION_ENTRY 500

#define MAX_UNKNOWN_WORD 500
...
commit_ochaire(struct seg_ent *seg, int count, xstr* xs)
{
  ...
  anthy_truncate_row(i * 2 + 1);
}

/* recordの領域を節約するために、お茶入れ学習のネガティブな
   エントリを消す(矛盾しないものは残すのは面倒なので全部消す) */
static void
...
static void
learn_ochaire_partial(struct splitter_context *sc,
		      struct segment_list *sl)
{
  int i;
  int count;
...
static void
learn_ochaire_full(struct splitter_context *sc,
		   struct segment_list *sl)
{
  struct seg_ent *head;
  xstr xs;
  if (sl->nr_segments > 4 || sc->char_count == 1 ||
      sc->char_count > 10) {
    return ;
  }
  head = anthy_get_nth_segment(sl, 0);
  xs = head->str;
  xs.len = sc->char_count;
  if (anthy_select_section("OCHAIRE_FULL_SEG", 1)) {
    return ;
  }
  commit_ochaire(head, sl->nr_segments, &xs);
  anthy_truncate_section(MAX_OCHAIRE_ENTRY_COUNT);
}
		  ...
		  struct splitter_context *sc)
{
  ...
  /* 部分文節列に対して */
  learn_ochaire_partial(sc, sl);
  /* 全入力に対して */
  learn_ochaire_full(sc, sl);
  learn_prediction(sl);
  learn_unknown(sl);
