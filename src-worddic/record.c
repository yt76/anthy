...
struct trie_node {
  ...
  /* 両端ループ */
  struct trie_node *lru_prev;
  struct trie_node *lru_next;
	     ...
	     struct record_section* rsc)
{
  ...
  while (!eol) {
	...
	{
	  xstr* xs;
	  xs = anthy_cstr_to_xstr(token + 1, ANTHY_UTF8_ENCODING);
...
write_quote_xstr(FILE* fp, xstr* xs)
{
  ...
  buf = (char*) alloca(xs->len * 6 + 2); /* UTF8 */
  anthy_sputxstr(buf, xs, ANTHY_UTF8_ENCODING);
...
read_session(struct record_stat *rst)
{
  ...
  while (!anthy_read_line(&tokens, &nr)) {
    ...
    /* 次にindex */
    xs = anthy_cstr_to_xstr(&tokens[0][1], ANTHY_UTF8_ENCODING);
	...
	str[strlen(str) - 1] = 0;
	xs = anthy_cstr_to_xstr(str, ANTHY_UTF8_ENCODING);
	free(str);
...
/* カラムを保存する */
static void
save_a_row(FILE *fp, struct record_row *c,
	   int dirty)
{
  int i;
  ...
  anthy_sputxstr(buf, &c->key, ANTHY_UTF8_ENCODING);
  /* index を出力 */
  fprintf(fp, "%s ", buf);
  /**/
...
static int
read_prediction_node(struct trie_node *n,
...
setup_filenames(const char *id, struct record_stat *rst)
{
  ...
  sprintf(rst->base_fn, "%s/.anthy/last-record1_%s.utf8",
	  home, id);
  /* 差分ファイル */
  rst->journal_fn = (char*) malloc(base_len +
				   strlen("/.anthy/last-record2_"));
  sprintf(rst->journal_fn, "%s/.anthy/last-record2_%s.utf8",
	  home, id);
}
