...
anthy_init_wtypes(void)
{
  ...
  anthy_type_to_wtype("#T35", &anthy_wt_t35);
}
...
int
anthy_wtype_get_id(const char *n)
{
  struct wttable *w;
  int i;
  for (w = wt_name_tab, i = 0; w->name; w++) {
    i++;
    if (!strcmp(w->name, n)) {
      return i;
    }
...
const char *
anthy_id_to_wtype(int id, wtype_t *t)
{
  struct wttable *w;
  if (id >= (int)sizeof(wt_name_tab)/sizeof(struct wttable)) {
    *t = anthy_wt_all;
    return NULL;
  }
...
int
anthy_wtype_get_vsuffix(wtype_t w)
{
  return w.wf & WF_VSUFFIX;
}
